...
def main():
    #message=input("Enter a message:") #taking message from user
    message=open('inputfile.txt','r') #import the txt file
    code=encoder(message.read())#calling function to encode the message from file
    decode=decoder(code)#calling function to decode the encode message
    message.seek(0)
    print('Original Message form file:\n',message.read()) #read the file
    message.close()#close file
    print('\nMessage after Encode:\n',code)#print the encode message
    print('\nMessage after decode the code:\n',decode)#print the decoded message
